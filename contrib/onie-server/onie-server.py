...
if sys.platform.startswith('win32'):
    def platform_get_ipaddrs():
        ...
        '''
        ipaddrs = []
        output = check_output(['ipconfig'], universal_newlines=True)
    ...
    def platform_get_ipaddrs():
        ...
        try:
            output = check_output(['ip', 'addr', 'show'], universal_newlines=True)
        except OSError:
            output = check_output(['ifconfig'], universal_newlines=True)
        for line in output.split('\n'):
...
dhcp_proc = subprocess.Popen(["python3", "dhcp-server.py", inside_ip, mask],
                             stdout=subprocess.PIPE,
                             stderr=subprocess.PIPE,
                             shell=False)

# Start a web server that listens on the INSIDE interface.
web_proc = subprocess.Popen(["python3", "web-server.py", inside_ip],
                            stdout=subprocess.PIPE,
                            stderr=subprocess.PIPE,
                            shell=False)
...
# INSIDE interface.
if (outside_ip):
    proxy_proc = subprocess.Popen(["python3", "proxy-server.py", inside_ip],
                                  stdout=subprocess.PIPE,
                                  stderr=subprocess.PIPE,
                                  shell=False)
else:
