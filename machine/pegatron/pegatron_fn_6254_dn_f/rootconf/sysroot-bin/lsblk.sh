    ...
    do
        ...
        do
            if [ -e ${first_block_device_link}/device ] && [ ${block_device_show} = false ]; then
                ...
                DEVICE_HCTL=$(readlink ${first_block_device_link}/device | awk -F "/" '{print $NF}')
                DEVICE_TYPE="disk"
...
$DEVICE_NAME  $DEVICE_HCTL    $DEVICE_TYPE $DEVICE_VENDOR $DEVICE_MODEL $DEVICE_REV $DEVICE_TRAN  $DEVICE_SIZE
EOF
)"
            fi
        done
...
echo "NAME HCTL       TYPE VENDOR   MODEL             REV TRAN SIZE$all_disks"
}
