...
if [ "$firmware" = "uefi" ] ; then
    ...
    delete_demo_partition="delete_demo_gpt_partition"
    ...
    delete_demo_partition="delete_demo_msdos_partition"
else
...
ping_test()
{
    local oob_device="eth2"
    local oob_ip="192.168.1.100"
    local remote_ip="192.168.1.254"
    
    if ifconfig ${oob_device} ${oob_ip}; then
       status="true"
    else
       status="false"
    fi
...
remove_diag_partition()
{
    local blk_dev="$1"
    local diag_part=$(sgdisk -p $blk_dev | grep "PEGATRON-DIAG" | awk '{print $1}')
    local diag_dev="$blk_dev$diag_part"
    ...
    # Format PEGATRON-DIAG data.
    echo 'y' | mkfs.ext4 ${diag_dev}
    
    # Format LOG-DIAG data.
    echo 'y' | mkfs.ext4 ${diag_log_dev}
    
    sgdisk -d $diag_log_part $blk_dev
    sgdisk -d $diag_part $blk_dev
    
    sgdisk --new=${demo_diag_part}:${demo_diag_start_sector}:${demo_diag_end_sector} \
        --attributes=${demo_diag_part}:=:$attr_bitmask \
        --change-name=${demo_diag_part}:$newer_volume_label $blk_dev
...
if [ "${status}" = "false" ]; then
	eval $delete_demo_partition $blk_dev > /dev/null
	echo "Ping test is failed on ONIE test"
	exit 0
fi

echo "
==== Reset to Default ===="
remove_diag_partition $blk_dev
eval $delete_demo_partition $blk_dev
onie-boot-mode -o none
