#-------------------------------------------------------------------------------
#
#  Copyright (C) 2021 Alex Doyle <adoyle@nivida.com>
...
PHONY += lvm2 lvm2-download lvm2-source lvm2-configure \
	lvm2-patch \
...
$(LVM2_PATCH_STAMP): $(LVM2_SOURCE_STAMP)
	$(Q) rm -f $@ && eval $(PROFILE_STAMP)
	$(Q) echo "====  Patching lvm2-$(LVM2_VERSION) ===="
...
$(LVM2_BUILD_STAMP): $(LVM2_CONFIGURE_STAMP) $(UTILLINUX_BUILD_STAMP)
	...
	$(Q) PATH='$(CROSSBIN):$(PATH)'	$(MAKE) -C $(LVM2_DIR) \
		CROSS_COMPILE=$(CROSSPREFIX) DESTDIR=$(DEV_SYSROOT) all
	$(Q) PATH='$(CROSSBIN):$(PATH)'	$(MAKE) -C $(LVM2_DIR) \
		CROSS_COMPILE=$(CROSSPREFIX) DESTDIR=$(DEV_SYSROOT) install
	$(Q) touch $@
